...
use emulator::{AddressType, EmuFile, Emulator, FaultType, Register, VmExit};
use emulator::{CoverageType, COVERAGE_ENTRY_EMPTY};
use jitcache::JitCache;
use mmu::{Perm, Section, VirtAddr, PERM_ACC, PERM_EXEC, PERM_READ, PERM_WRITE};
...
use basic_mutator::{EmptyDatabase, InputDatabase, Mutator};
    ...
    match num {
        ...
        63 => {
            ...
            if let Some(Some(EmuFile::FuzzInput { ref mut cursor })) = file {
                ...
                let result_cursor =
                    core::cmp::min(cursor.saturating_add(len), emu.fuzz_input.len());

                // Write in the bytes
                emu.memory
                    .write_from(VirtAddr(buf), &emu.fuzz_input[*cursor..result_cursor])?;
        ...
        1024 => {
            ...
            let bytes = emu
                .memory
                .peek(VirtAddr(filename), fnlen, Perm(PERM_READ))?;
    ...
    loop {
        ...
        while (rdtsc() - it) < 500_000_000 {
            ...
            emu.reset(&*original, &corpus, |emu| {
                let perms = emu.memory.peek_perms(VirtAddr(buf as usize), len).unwrap();
            ...
            if (rng.rand() % 32) != 0 && corpus.inputs.len() > 0 {
                ...
                if let Some(input) = corpus.inputs.get(sel) {
                    // Copy the input
                    options =
                        u32::from_ne_bytes(input.data[input.data.len() - 4..].try_into().unwrap());
                    mutator
                        .input
                        .extend_from_slice(&input.data[..input.data.len() - 4]);
            ...
            if let Some(repro_file) = REPRO_MODE {
                emu.fuzz_input.clear();
                emu.fuzz_input.extend_from_slice(
                    &std::fs::read(repro_file).expect("Failed to read repro file"),
                );
            }

            // Inject the input
            emu.memory
                .write_from(VirtAddr(buf as usize), &emu.fuzz_input)
                .unwrap();
            emu.set_reg(Register::A1, emu.fuzz_input.len() as u64);

            let vmexit = loop {
                let vmexit = emu
                    .run(&mut run_instrs, &mut local_stats.vm_cycles, &*corpus)
            ...
            if let Some((fault_type, vaddr)) = vmexit.is_crash() {
                ...
                corpus.unique_crashes.entry_or_insert(&key, pc.0, || {
                    ...
                    let crash_fn = Path::new("crashes").join(format!(
                        "{:#x}_{:?}_{:?}.crash",
                        (key.0).0,
                        key.1,
                        key.2
                    ...
                    print!("New crash {:?}\n", crash_fn);
                    std::fs::write(&crash_fn, &emu.fuzz_input).expect("Failed to write fuzz input");
                    std::fs::write(&reg_fn, &format!("{}", emu))
...
impl InputDatabase for Corpus {
    ...
    fn input(&self, idx: usize) -> Option<&[u8]> {
        self.inputs.get(idx).map(|x| &x.data[..x.data.len() - 4])
    }
}
...
fn malloc_bp(emu: &mut Emulator) -> Result<(), VmExit> {
    if let Some(alc) = emu.memory.allocate(emu.reg(Register::A1) as usize) {
        ...
        if VERBOSE_ALLOCS {
            print!(
                "malloc returned {:#018x} - size was {:#x}\n",
                alc.0,
                emu.reg(Register::A1)
            );
        }
...
fn _calloc_bp(emu: &mut Emulator) -> Result<(), VmExit> {
    ...
    if VERBOSE_ALLOCS {
        print!(
            "calloc returned {:#018x} - size was {:#x}\n",
            result.0,
            size * nmemb
        );
    }
...
fn _realloc_bp(emu: &mut Emulator) -> Result<(), VmExit> {
    ...
    let old_size = if old_alc == VirtAddr(0) {
        ...
        emu.memory
            .get_alc(old_alc)
            .ok_or(VmExit::InvalidFree(old_alc))?
    };
    ...
    let new_alc = emu
        ...
        .and_then(|new_alc| {
            if VERBOSE_ALLOCS {
                print!(
                    "realloc {:#018x} -> {:#018x} - size {:#x} -> {:#x}\n",
                    old_alc.0, new_alc.0, old_size, size
                );
            }
            ...
            if old_alc != VirtAddr(0) {
                ...
                emu.memory
                    .free(old_alc)
                    .expect("Failed to free old allocation?");
            }
...
pub fn load_elf<P: AsRef<Path>>(filename: P, emu: &mut Emulator) -> io::Result<()> {
    ...
    let stdout =
        core::str::from_utf8(&output.stdout).expect("Failed to get readelf stdout as a string");
    ...
    for line in stdout.lines() {
        if line.starts_with("Entry point 0x") {
            ...
            entry_point =
                Some(u64::from_str_radix(&line[14..], 16).expect("Entry point line malformed"));
        } else {
            ...
            let offset = info
                .next()
                .and_then(|x| usize::from_str_radix(&x[2..], 16).ok())
    ...
    let stdout = core::str::from_utf8(&output.stdout).expect("Failed to get nm stdout as a string");

    // Parse NM output
    for line in stdout.lines() {
        let mut info = line.split_whitespace();
        if info.clone().count() != 3 {
            continue;
        }
...
fn main() -> io::Result<()> {
    ...
    let corpus = Arc::new(Corpus {
        ...
        coverage_log: Mutex::new(
            File::create("coverage.txt").expect("Failed to create coverage file"),
        ),
        lighthouse_log: Mutex::new(
            File::create("lighthouse.txt").expect("Failed to create lighthouse coverage file"),
        ),
        compile_jobs: Default::default(),
        coverage_table: (0..32 * 1024 * 1024)
            .map(|_| (AtomicU64::new(COVERAGE_ENTRY_EMPTY), AtomicU64::new(0)))
            .collect(),
    });
    ...
    loop {
        ...
        let mut tmp = 0;
        let vmexit = emu
            .run_emu(&mut tmp, &*corpus)
            .expect_err("Failed to execute emulator");

        match vmexit {
            VmExit::Snapshot => {
                emu.remove_breakpoint(emu.resolve_symbol(FUZZ_START_SYM).unwrap());
                break;
            }
            VmExit::Syscall => {
                print!("Syscall {}\n", emu.reg(Register::A7));
    ...
    let num_cores = if REPRO_MODE.is_some() { 1 } else { 192 };
